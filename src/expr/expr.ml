...
let rec fmt1 (e : expr) : string =
  ...
  | Var x  -> x
  | Let(x, erhs, ebody) ->
      String.concat ~sep:" " ["let"; x; "="; fmt1 erhs;
                              "in"; fmt1 ebody; "end"]
  | Prim(ope, e1, e2) ->
      String.concat ~sep:"" ["("; fmt1 e1; ope; fmt1 e2; ")"]

(* Format expressions as strings, avoiding excess parentheses *)
let rec fmt2 (ctxpre : int) (e : expr) : string =
  match e with
  | CstI i -> string_of_int i
  | Var x  -> x
  | Let(x, erhs, ebody) ->
      String.concat ~sep:" "
        ["let"; x; "="; fmt2 (-1) erhs; "in"; fmt2 (-1) ebody; "end"]
  | Prim(op, e1, e2) ->
      (match op with
...
and wrappar ctxpre pre ss =
  if pre <= ctxpre then String.concat ~sep:"" ("(" :: ss @ [")"])
...
let rec closedin (e : expr) (env : string list) : bool =
  match e with
  | CstI i -> true
  | Var x  -> List.exists ~f:(fun y -> x=y) env
  | Let(x, erhs, ebody) ->
      let env1 = x :: env in
...
(* Compiling from expr to texpr. The compile-time environment cenv is a list of
   variable names; the position of a variable in the list indicates its binding
   depth and hence the position in the runtime environment. The integer giving
   the position is the same as a deBruijn index in the lambda calculus: the
   number of binders between this occurrence of a variable, and its binding. *)
...
(* Storing intermediate results and variable bindings in the same stack.
   ...
   We can store intermediate results as well as variable bindings on the same
   stack. This is possible because the scopes of variables are statically
   nested: the scope of a variable (and hence the need for its binding) does
   not extend beyond the expression in which it is bound. *)
type sinstr =
...
let rec seval (inss : sinstr list) stack =
  ...
  | (ins0 :: rest), _ ->
      ...
      | SVar i,              _ -> seval rest (List.nth_exn stack i :: stack)
...
(* To compile for the single-stack machine, we must keep count (at
   compile-time) of the intermediate results, interspersed between the variable
   bindings on the stack.  For this we need a compile-time variable environment
   with dummies Intrm for intermediate values. *)
type rtvalue =
...
(* Correctness: eval e [] [] equals seval (scomp e []) [] for an expression
   with no free variables. *)

(* Net effect principle: when c = scomp e {} is the code generated for
   expression e, then the net result of evaluating c is to push the value of e
   onto the evaluation stack.

   Hence, if v = eval e [] [] then for any stack stk, seval (scomp e []) s
   equals  v :: s *)
